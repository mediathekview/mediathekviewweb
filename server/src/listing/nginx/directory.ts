...
import { File, Directory, RessourceType } from '../';
...
export class NginxDirectory implements Directory {
    ...
    private fetch(): Promise<ParseResult> {
        if (this.fetchPromise == null) {
            this.fetchPromise = new Promise<ParseResult>(async (resolve, reject) => {
                const response = await HttpClient.get(this.ressource);
    ...
    private parseMatch(match: RegExpExecArray): { directory: NginxDirectory | null, file: HttpFile | null } {
        ...
        const url = URL.resolve(this.ressource, href);
        ...
        if (url.endsWith('/')) {
            ...
            file = new HttpFile({ ressource: { uri: url, type: RessourceType.Http }, name: name, size: parsedSize, date: parsedDate });
        }
